...
visited = {k: 0 for k, v in modules["xn"].memory.items()}
cycles = {}
print(visited)
...
def cycle(q, count):
    ...
    cnt_high = 0
    while q:
        org, mod, signal = q.pop(0)

        if str(mod) == "xn" and signal == "hi":
            visited[org] += 1
            cycles[org] = count + 1
            if all([x == 1 for x in visited.values()]):
        ...
        match mod.type:
            case "broadcaster":
                for t in mod.targets:
                    q.append((str(mod), modules[t], signal))
            case "start":
                for t in mod.targets:
                    q.append((str(mod), modules[t], signal))
            case "%":
...
for i in range(10000):
    l, h = cycle([("button", modules["broadcaster"], "lo")], i)
